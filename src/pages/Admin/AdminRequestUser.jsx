...
function AdminRequestUser() {
  ...
  // MODAL STATUS
  const [modalStatus, setModalStatus] = useState(false);
  const [modalStatusRequest, setModalStatusRequest] = useState(null);
  ...
  // Funzione per chiudere il modal e resettare i campi
  const closeModal = () => {
    setModalStatus(false);
    setImgCompleted(null);
    setBodyCompleted("");
    setBodyQuote("");
    setSuccessEmailDefault("");
    setSuccessEmail("");
  ...
  const handleActionStatus = async () => {
    ...
    // 1. MODIFICA STATO (Aggiorna la tabella all'istante se ha successo)
    try {
      const updateStatus = await fetch(`https://winelabelmaker-be.onrender.com/api/Request/updateAdmin/${modalStatusRequest.idRequest}`, {
        method: "PUT",
        headers: {
      ...
      // Forza l'aggiornamento della tabella in tempo reale
      await userRequest();
    } catch (error) {
      console.error(error);
      setError("Impossibile aggiornare lo stato: " + error.message);
      setTimeout(() => setError(""), 2000);
      return; // Si ferma qui se il database fallisce
    }

    // 2. INVIO EMAIL (Isolato: se va in errore 500, la pagina NON si blocca)
    try {
      // Email Default
      if (newStatus === 1 || newStatus === 3 || newStatus === 5) {
        setSuccessEmailDefault("Email inviata con successo!");
      ...
      if (newStatus === 2) {
        ...
        if (!emailQuoteStatus.ok) throw new Error("Errore invio email preventivo");
        setSuccessEmailQuote("Email preventivo inviata con successo!");
      }

      // Se tutto va a buon fine chiude il modal dopo 1.5 secondi per mostrare il successo
      setTimeout(() => {
        closeModal();
      }, 1500);
    } catch (emailError) {
      console.error("Errore Email (500):", emailError);

      // Mostra l'errore a schermo ma chiude comunque il modal senza bloccare l'interfaccia
      setError("Stato modificato, ma l'invio della mail è fallito (Errore 500)");
      setTimeout(() => {
        setError("");
        closeModal(); // Chiude il modal pulendo i campi
      }, 3000);
    }
  };
  ...
  // STATE MANAGEMENT
  const isStatusDisabled = (disabledStatus) => {
    if (!modalStatusRequest) return true;
    const currentStatus = modalStatusRequest.status;
      ...
      <Container className="mt-5">
        <Row>
          <Col xs={12} md={12} lg={12}>
            ...
            {error && (
              <Alert variant="warning" className="fs-5 fw-bold text-center my-3">
                {error}
              </Alert>
            )}
                          ...
                          <div className="d-flex align-items-center flex-column">
                            ...
                            <div className="d-flex gap-1">
                              <Button
                                ...
                                onClick={() => {
                                  setModalStatusRequest(request);
                                  setNewStatus(statusToNumber[request.status]);
                                  setModalStatus(true);
                                }}
                              >
                                <PencilSquare />
            ...
            {/* MODAL STATUS*/}
            <Modal show={modalStatus} onHide={closeModal} centered>
              <Modal.Header closeButton>
                <Modal.Title className="bubbler-one-regular text-black fw-bold">Stato della richiesta</Modal.Title>
              </Modal.Header>
