...
function AdminRequestUser() {
  ...
  const handleActionStatus = async () => {
    ...
    try {
      ...
      if (newStatus === 1 || newStatus === 3 || newStatus === 5) {
        setSuccessEmailDefault("Email inviata con successo!");
        setTimeout(() => setSuccessEmailDefault(""), 2000);
      }
      ...
      if (newStatus === 4) {
        ...
        const emailResponse = await fetch("https://winelabelmaker-be.onrender.com/api/Email/completed", {
          ...
          body: JSON.stringify({ requestId: modalStatusRequest.idRequest, imageName: imgCompleted.name, customBody: bodyCompleted }),
        });
      ...
      if (newStatus === 2) {
        if (!bodyQuote || bodyQuote.trim() === "") {
          setErrorEmailQuote("Il testo della mail è obbligatorio");
          setTimeout(() => setErrorEmailQuote(""), 2000);
          return;
        }
        const emailQuoteStatus = await fetch("https://winelabelmaker-be.onrender.com/api/Email/sendQuote", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
          body: JSON.stringify({ requestId: modalStatusRequest.idRequest, customBody: bodyQuote }),
        });
        if (!emailQuoteStatus.ok) throw new Error("Errore invio email preventivo");

        setSuccessEmailQuote("Email preventivo inviata con successo!");
        setTimeout(() => setSuccessEmailQuote(""), 2000);
      }

      // Successo finale
      userRequest();
      setTimeout(() => setModalStatus(false), 2000);
    } catch (error) {
      console.error(error);
      setError(error.message);
